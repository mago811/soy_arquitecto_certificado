...
- 2026-08-18 (análisis del mock): `data/plan.json > objetivos` tiene
  `context-mgmt` como zonaInicial **verde / baseline 100**, heredado del
  diagnóstico del primer intento. El mock externo lo mide en **40%** — es el peor
  objetivo y el semáforo lo muestra como fortaleza. Hay que corregir el baseline
  y probablemente desglosar `context-mgmt` en objetivos más finos (facts block,
  lost-in-the-middle, provenance, session isolation). OJO: cambiar ids de
  objetivos o de tareas rompe el tachado guardado en localStorage — pendiente de
  decisión explícita de Mago, no lo toqué. Detalle en
  `notes/analisis-mock-2026-08-18.md`.
